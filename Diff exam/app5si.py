...
# === SECURE SECRETS (Waldonia + Stripe) ===
...
# -------------------------------------------------
# PROFESSIONAL CSS – WIX & MOBILE
# -------------------------------------------------
st.markdown("""
<style>
    /* Core */
    .main {background:#ffffff;padding:1.5rem 1rem;max-width:1400px;margin:auto;}
    [data-testid="stSidebar"] {display:none;} /* Hide sidebar on mobile */
    ...
    .hero {text-align:center;padding:2rem 1rem;background:linear-gradient(135deg,#f0fdf4,#dcfce7);border-radius:20px;margin-bottom:2rem;}
...
with st.container():
    ...
    with col2:
        ...
        if basket.empty:
            ...
            with c3: st.markdown(f'<div class="stat-card"><div class="stat-num">{eco:,.0f}</div><div class="stat-label">Eco kg</div></div>', unsafe_allow_html=True)
...
# === PLANT TREES ===
if save > 0:
    st.markdown("#### Plant Trees to Offset")
    trees_to_plant = st.slider("Trees to Plant", 1, 100, int(trees))
    ...
    if sid and stripe.checkout.Session.retrieve(sid).payment_status == "paid":
        ...
        if result:
            ...
            st.download_button("Certificate", f"EcoGigHub Certificate\nID: {impact['id']}\nTrees: {trees_to_plant}\nDate: {impact['date'][:10]}", "certificate.txt")
            st.balloons()
...
<div class="footer">
    ...
    <em>Embed on Wix • Mobile Ready • Business Impact</em>
</div>
""", unsafe_allow_html=True)