...
# -------------------------------------------------
st.set_page_config(
    page_title="EcoGigHub – Business CO₂ Solutions",
    page_icon="leaf",
    layout="wide",
...
# -------------------------------------------------
# CSS – PROFESSIONAL & MOBILE
# -------------------------------------------------
st.markdown("""
<style>
    .main {background:#ffffff;padding:1.5rem 1rem;max-width:1400px;margin:auto;}
    [data-testid="stSidebar"] {display:none;}
    .css-1d391kg {font-family:'Inter', 'Segoe UI', sans-serif !important;}

    h1 {color:#1e3a8a !important;font-weight:800;font-size:2.2rem !important;text-align:center;}
    ...
    .stat-card {
        ...
        border:1px solid #e2e8f0;box-shadow:0 6px 16px rgba(0,0,0,0.05);transition:0.3s;
    }
    .stat-card:hover {transform:translateY(-4px);box-shadow:0 12px 24px rgba(0,0,0,0.08);}
    .stat-num {font-size:2.8rem;font-weight:900;color:#166534;margin:0;}
    ...
    .badge-provider {background:#1e40af;color:white;padding:6px 14px;border-radius:20px;font-weight:700;font-size:0.85rem;display:inline-block;}
...
def create_payment(amount, desc):
    try:
        session = stripe.checkout.Session.create(
            ...
            line_items=[{'price_data': {'currency': 'usd', 'product_data': {'name': 'EcoGigHub Impact'}, 'unit_amount': amount}, 'quantity': 1}],
...
<div class="hero">
    <h1>EcoGigHub</h1>
    <p>Reduce emissions. Grow your business. Plant real trees with <strong>Waldonia</strong>.</p>
</div>
""", unsafe_allow_html=True)
...
# === BUY OR DONATE – WITH WALDONIA BADGE ===
if save > 0:
    st.markdown("## Take Action")
    
    action = st.radio(
        "Choose your impact",
        ["Buy Eco Products & Services", "Donate to Plant Trees (Waldonia)"],
        horizontal=True
    )

    if action == "Buy Eco Products & Services":
        st.info("Complete your purchase to reduce emissions.")
        if st.button("PROCEED TO CHECKOUT", type="primary"):
            url = create_payment(int(cost * 100), "EcoGigHub Business Purchase")
            if url: st.markdown(f"[Pay Now]({url})")

    else:
        st.markdown("#### Plant Real Trees with <span class='badge-provider'>Waldonia</span>", unsafe_allow_html=True)
        ...
        note = st.text_input("Company / Name (on certificate)", "EcoHero")

        col1, col2 = st.columns(2)
        with col1: st.metric("Annual CO₂ Offset", f"{trees_to_plant * TREE_CO2_YEAR:,.0f} kg")
        with col2: st.metric("Donation", f"${cost_trees:.2f}")

        if st.button(f"PLANT {trees_to_plant} TREES WITH WALDONIA", type="primary"):
            url = create_payment(int(cost_trees * 100), f"{trees_to_plant} Trees - {note}")
            if url: st.markdown(f"[Pay & Plant via Waldonia]({url})")
    ...
    if sid:
        try:
            ...
            if session.payment_status == "paid":
                with st.spinner("Planting trees with Waldonia..."):
                    result = plant_trees(trees_to_plant, note)
                    if result:
                        impact = {"id": result.get("order_id"), "trees": trees_to_plant, "provider": "Waldonia", "date": datetime.now().isoformat()}
                        st.session_state.impacts.append(impact)
                        st.success(f"Planted {trees_to_plant} trees with Waldonia!")
                        st.download_button(
                            "Download Certificate",
                            f"EcoGigHub x Waldonia Certificate\nID: {impact['id']}\nTrees: {trees_to_plant}\nProvider: Waldonia\nDate: {impact['date'][:10]}",
                            f"waldonia_certificate_{impact['id']}.txt",
                            "text/plain"
                        )
                        st.balloons()
                        st.rerun()
...
if st.session_state.impacts:
    st.markdown("### Your Tree Impact (Waldonia)")
    for imp in st.session_state.impacts[-3:]:
        st.info(f"Planted **{imp['trees']} trees** with **Waldonia** | ID: `{imp['id']}` | {imp['date'][:10]}")
...
st.markdown("""
<div class="footer">
    <strong>EcoGigHub</strong> – Trees Planted by <strong>Waldonia</strong> • Data: IPCC<br>
    <em>Wix-Ready • Mobile • Real Impact</em>
</div>
""", unsafe_allow_html=True)